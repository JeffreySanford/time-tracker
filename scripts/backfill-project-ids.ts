...
import {
  CommitSession,
  CommitSessionSchema,
} from '../apps/api/src/commitsession.schema';
import {
  CommitWorkLog,
  CommitWorkLogSchema,
} from '../apps/api/src/commitworklog.schema';
...
async function main() {
  ...
  const commitWorkLogModel = mongoose.model<CommitWorkLog>(
    'CommitWorkLog',
    CommitWorkLogSchema,
    'commit_work_logs',
  );
  const commitSessionModel = mongoose.model<CommitSession>(
    'CommitSession',
    CommitSessionSchema,
    'commit_sessions',
  );
  ...
  const service = new GitIngestService(
    commitWorkLogModel as unknown as Model<CommitWorkLog>,
    commitSessionModel as unknown as Model<CommitSession>,
    projectModel as unknown as Model<Project>,
  );
